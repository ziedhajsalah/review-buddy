...
**v0.3.1 — Phase 1 complete** (one-way viewer), installed with zero build steps (the viewer ships prebuilt). What's in:
...
- **Viewer** — Vite + React 19 + Tailwind v4 on [`@pierre/diffs`](https://www.npmjs.com/package/@pierre/diffs): Prologue/Description overview, risk-rated chapter cards, split-pane chapter review, unified/split diffs with line/word/char highlighting granularity + off-thread syntax highlighting (worker pool), per-file controls (marking a file viewed collapses it), collapse state that persists across chapters, cookie-persisted display prefs. Agent prose and the PR description render as **markdown** (`react-markdown` + GFM, no raw HTML).
